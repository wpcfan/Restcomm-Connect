...
public final class Bootstrapper extends SipServlet implements
SipServletListener {
    ...
    private ActorRef gateway(final Configuration configuration,
        ...
        final Configuration settings = configuration
                .subset("media-server-manager");
        final ActorRef gateway = system.actorOf(new Props(
                new UntypedActorFactory() {
                    private static final long serialVersionUID = 1L;

                    @Override
                    public UntypedActor create() throws Exception {
                        final String classpath = settings
                                .getString("mgcp-server[@class]");
                        return (UntypedActor) new ObjectFactory(loader)
        ...
        final String timeout = settings
                .getString("mgcp-server.response-timeout");
    ...
    public void servletInitialized(SipServletContextEvent event) {
        if (event.getSipServlet().getClass().equals(Bootstrapper.class)) {
            ...
            try {
                ...
                logger.error(
                        "ObjectInstantiationException during initialization",
                        exception);
            }
            ...
            try {
                ...
                logger.error("UnknownHostException during initialization",
                        exception);
            }
