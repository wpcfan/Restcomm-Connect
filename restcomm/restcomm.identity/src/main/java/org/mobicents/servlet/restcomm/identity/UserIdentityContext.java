...
public class UserIdentityContext {
    ...
    public UserIdentityContext(HttpServletRequest request, AccountsDao accountsDao, OrganizationsDao organizationsDao) {
        ...
        if (accountKey != null) {
            if (accountKey.isVerified()
                    && isOrganizationCompliant(accountKey.getAccount(), UriUtils.getHttpRequestUrl(request), organizationsDao)) {
    ...
    private boolean isOrganizationCompliant(Account account, URI requestUrl, OrganizationsDao organizationsDao) {
        if (requestUrl != null) {
            String namespace = OrganizationUtils.getOrganizationNamespace(requestUrl.getHost());
            if (namespace != null) {
                // Assuming host as a domain name, proceed validating Organizations
