 ...
 *
 */
public class Jsr309MediaGatewayMock extends MediaGatewayMock {
    
    private final ActorSystem actorSystem;
    private final MediaServerControllerFactory mscontrolFactory;

    public Jsr309MediaGatewayMock(String name, InetAddress address, int port, int timeout) {
        this.actorSystem = ActorSystem.create("JSR309 Media GW");
        this.info = new MediaServerInfo(name, address, port, timeout);
        this.mscontrolFactory = new Jsr309ControllerFactory(system, info, msControlFactory);
    }
